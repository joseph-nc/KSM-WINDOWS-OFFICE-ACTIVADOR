...
cls
color 0D
title  JOSEPH WIN^&OFFI Activador
mode 76, 30
...
call :dk_color2 %Magenta% "Check this page for help" %_Yellow% "  "
)
)
...
::  Check   to see how it's generated
...
echo.       ______________________________________________________________
echo.
call :_color2 %_White% "           " %_Green% "Enter a menu option in the Keyboard [1,2,3,4,5,7,0]"
choice /C:12345670 /N
set _el=%errorlevel%

if %_el%==8 exit /b
if %_el%==7 (if %vNextOverride% EQU 0 (set vNextOverride=1) else (set vNextOverride=0))&goto _KMS_Menu
if %_el%==600998 (if %_Debug%==0 (set _Debug=1) else (set _Debug=0)) &goto _KMS_Menu
...
if %ActWindows% EQU 0 (
  ...
  if %SSppHook% EQU 0 (
    ...
    echo.&echo Failed checking KMS Activation ID^(s^) for Windows.&echo Check this page for help   &call :CheckWS
    exit /b
    )
  )
)
...
call :_color2 %Magenta% "Check this page for help" %_Yellow% "  "
exit /b
